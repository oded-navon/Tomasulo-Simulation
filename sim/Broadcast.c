...
void broadcast_to_load_buffers(char* unit_to_broadcast_name);
...
void broadcast_result(char* rs_or_buff_name, float value)
{
	...
	broadcast_to_load_buffers(rs_or_buff_name);
}
...
void broadcast_to_rs(int num_of_rs_stations, float operation_result, RS* rs_stations, char* unit_to_broadcast_name)
{
	for (int i = 0; i < num_of_rs_stations; i++)
	{
		...
		if (rs_stations[i].occupied && strcmp(rs_stations[i].dst_waiting, unit_to_broadcast_name) == 0)
		{
...
//This function goes over all the load buffers and finds all the buffers which are waiting fot the
//value sent in the broadcast
void broadcast_to_load_buffers(char* unit_to_broadcast_name)
{
	for (int i = 0; i < _config_args_read->mem_nr_load_buffers; i++)
	{
		if (load_buffers[i].occupied && strcmp(load_buffers[i].dst_waiting, unit_to_broadcast_name) == 0)
		{
			memset(load_buffers[i].dst_waiting, 0, NAME_LEN);
		}
	}
}
